...
-- Cheap insurance against the Ctrl+H/Backspace byte collision the upstream
-- README calls out (0x08 means both, depending on keyboard-protocol
-- negotiation) — dotfiles-local patch, not upstream. The actual bug hit here
-- was LazyVim's own <C-h/j/k/l>=<C-w>h/j/k/l keymaps loading on the
-- `VeryLazy` event and clobbering the maps above; see the VeryLazy autocmd
-- in nvim/.config/nvim/lua/plugins/vim_tmux_navigator.lua that dofiles this.
map("<BS>", "h", "left", "Navigate left (vim/herdr)")
